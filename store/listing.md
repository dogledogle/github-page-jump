# Chrome Web Store 上架材料（GitHub Page Jump v1.3.0）

> 上传包：`dist/github-page-jump-1.3.0.zip`
...
• Fixed pagination — use the toolbar popup to pin the paginator near the bottom of the viewport.
...
PRIVACY
• Only the storage permission is requested, solely to remember the fixed-pagination preference.
...
- **Permission justification（storage）**:
  The extension stores one boolean preference that controls whether pagination is fixed near the bottom of the viewport. No personal data or page content is stored.
...
- **Data usage disclosures**: 全部选 **No**
  ...
  - Does this item collect or use locally stored data? → No（只在 `chrome.storage.sync` 写入扩展自身的非个人布尔偏好，不读取 cookie/localStorage）
...
2. Developer Dashboard → **New item** → 上传 `dist/github-page-jump-1.3.0.zip`。
...
- [x] manifest 只申请 `storage` 权限、无远程代码
- [x] 图标 16/32/48/128 已在 manifest 声明且文件存在
- [ ] 上传前用浏览器重载一次 1.3.0 做最后冒烟（首末页跳转、输入跳转、固定模式、暗色主题）
- [ ] Dashboard 提交
